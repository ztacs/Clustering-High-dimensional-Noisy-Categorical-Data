...
function [correct_cn,indicator, cluster_center,cluster_all, total, pgain] = ccdv2_original_observed(data)
...
while 1   
    ...
    for i = 1:sw(1)
        for j = 1: length(data_with_frequency(:,1))
            d = dist_cate_observed(working(i,1:m), data_with_frequency(j,1:m));
            for k = 1:cn
                if d == k
   ...
   elseif (centercd(radius0 - 1) - centercd(radius0)) >= 2
    ...
    if max_chi >= chi2inv(.95,m) %& indicator < 10
        ...
        for ii = 1:sw(1)
            if dist_cate_observed(center(1:m),working(ii,1:m)) <= radius%(indicator)%5%time(indicator) % the criteria of a cluster is 2 temporarily
...
for i = 1:length(ori(:,1))
    ...
    for j = 1:length(cluster_center(:,1))
        dd = [dd dist_cate_observed(ori(i,1:m),cluster_center(j,1:m))];
    end
