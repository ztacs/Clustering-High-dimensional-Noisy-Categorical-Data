function [R_hat cat] =unit_circle_encoding(data)
 
% Equilateral Simplex Encoding for Nominal Data
%1. data: The dataset to be encoded; the last column needs to be the ture cluster label;
%2. R_hat: Encoded nominal data by Equilateral Simplex Encoding; missings are
%coded as 0 vectors. 
%cat: number of unque values (not include NaN) of each attribute.
m = length(data(1,:)) - 1;%m: # of attributes
n = length(data(:,1));%n: sample size
